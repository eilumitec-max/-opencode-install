...
create table if not exists tenants (
  ...
  logo text not null, primary_color text not null, whatsapp text not null,
  address text not null, delivery_fee decimal(10,2) not null default 0,
  min_order decimal(10,2) not null default 0, working_hours text not null,
  installments text not null, created_at timestamptz default now()
);

create table if not exists categories (
  id text not null, tenant_id text not null references tenants(id) on delete cascade,
  name text not null, icon text not null default '📦', active boolean default true,
  "order" integer default 0, created_at timestamptz default now(),
  primary key (id, tenant_id)
);

create table if not exists products (
  id text not null, tenant_id text not null references tenants(id) on delete cascade,
  name text not null, category text not null, price decimal(10,2) not null,
  old_price decimal(10,2), stock integer default 0, image text default '',
  active boolean default true, featured boolean default false, sales integer default 0,
  created_at timestamptz default now(), primary key (id, tenant_id)
);
...
create table if not exists orders (
  ...
  payment text not null, method text not null, date text not null, address text not null,
  created_at timestamptz default now(), primary key (id, tenant_id)
);

-- 2. AUTH (tenant_users table)
...
create table if not exists customers (
  ...
  tenant_id text not null references tenants(id) on delete cascade,
  cep text default '', address text default '', number text default '',
  complement text default '', neighborhood text default '', city text default '',
  state text default '', created_at timestamptz default now(),
...
drop policy if exists "Anon all access" on tenants;
drop policy if exists "Anon all access" on categories;
drop policy if exists "Anon all access" on products;
drop policy if exists "Anon all access" on orders;
drop policy if exists "Users can read own link" on tenant_users;
drop policy if exists "Users can insert own link" on tenant_users;
...
('c1', '1', 'Açaís', '🍇', true, 1), ('c2', '1', 'Copos', '🥤', true, 2),
('c3', '1', 'Sorvetes', '🍦', true, 3), ('c4', '1', 'Coberturas', '🍫', true, 4),
('c5', '1', 'Frutas', '🍓', true, 5), ('c1', '2', 'Gelatos', '🍦', true, 1),
('c2', '2', 'Bebidas', '🧃', true, 2), ('c3', '2', 'Sobremesas', '🍰', true, 3),
('c1', '3', 'Açaís', '🍇', true, 1), ('c2', '3', 'Bebidas', '🧃', true, 2),
